...
import { t as BrowserPerformanceMeasurement } from "./BrowserPerformanceMeasurement-MGF07V7p.js";
...
function generateAppMetadataKey({ environment, clientId }) {
	return [
		...
		environment,
		clientId
	].join(CACHE_KEY_SEPARATOR$1).toLowerCase();
}
...
function getStandardAuthorizeRequestParameters(authOptions, request, logger, performanceClient) {
	...
	if (request.state) addState(parameters, request.state);
	if (request.claims || authOptions.clientCapabilities && authOptions.clientCapabilities.length > 0) addClaims(parameters, request.claims, authOptions.clientCapabilities);
...
var AsyncMemoryStorage = class {
	...
	handleDatabaseAccessError(error, correlationId) {
		if (error instanceof BrowserAuthError && error.errorCode === databaseUnavailable) this.logger.error("1wx7zz", correlationId);
		else throw error;
	}
	/**
...
function getCookieExpirationTime(cookieLifeDays) {
	...
	return new Date(today.getTime() + cookieLifeDays * COOKIE_LIFE_MULTIPLIER).toUTCString();
}
...
var LocalStorage = class {
	...
	*/
	clear() {
		...
		this.getKeys().forEach((cacheKey) => {
			if (cacheKey.startsWith(PREFIX) || cacheKey.indexOf(this.clientId) !== -1) this.removeItem(cacheKey);
		});
	}
	/**
...
var BrowserCacheManager = class extends CacheManager {
	...
	*/
	trackVersionChanges(correlationId) {
		...
		if (previousVersion !== version) this.setItem(VERSION_CACHE_KEY, version, correlationId);
	}
	/**
	...
	*/
	setItem(key, value, correlationId) {
		...
		for (let i = 0; i <= maxRetries; i++) try {
			...
			if (i > 0) for (let schemaVersion = 0; schemaVersion <= CREDENTIAL_SCHEMA_VERSION; schemaVersion++) {
			...
			if (cacheError.errorCode === cacheQuotaExceeded && i < maxRetries) {
				if (!accessTokenKeys.length) for (let i = 0; i <= CREDENTIAL_SCHEMA_VERSION; i++) if (key === getTokenKeysCacheKey(this.clientId, i)) {
	...
	*/
	async setUserData(key, value, correlationId, timestamp, kmsi) {
		...
		for (let i = 0; i <= maxRetries; i++) try {
			...
			const cacheError = createCacheError(e);
			if (cacheError.errorCode === cacheQuotaExceeded && i < maxRetries) {
				if (!accessTokenKeys.length) for (let i = 0; i <= CREDENTIAL_SCHEMA_VERSION; i++) {
					const tokenKeys = this.getTokenKeys(i).accessToken;
					accessTokenKeys.push(...tokenKeys);
					tokenKeysCount[i] = tokenKeys.length;
				}
	...
	*/
	clear(correlationId) {
		...
		this.temporaryCacheStorage.getKeys().forEach((cacheKey) => {
			if (cacheKey.indexOf(PREFIX) !== -1 || cacheKey.indexOf(this.clientId) !== -1) this.removeTemporaryItem(cacheKey);
		});
		this.browserStorage.getKeys().forEach((cacheKey) => {
			if (cacheKey.indexOf(PREFIX) !== -1 || cacheKey.indexOf(this.clientId) !== -1) this.browserStorage.removeItem(cacheKey);
		});
	...
	*/
	generateCacheKey(key) {
		if (StringUtils.startsWith(key, PREFIX)) return key;
	...
	*/
	async hydrateCache(result, request) {
		const cacheRecord = {
			idToken: createIdTokenEntity(result.account.homeAccountId, result.account.environment, result.idToken, this.clientId, result.tenantId),
	...
	*/
	async handleCodeResponse(response, request, apiId) {
		...
		try {
			...
			if (e instanceof ServerError && e.subError === userCancelled) throw createBrowserAuthError(userCancelled);
			else throw e;
		}
	...
	*/
	async acquireToken(silentRequest) {
		...
		try {
			...
			if (error instanceof BrowserAuthError && error.errorCode === cryptoKeyNotFound) this.logger.verbose("06wena", this.correlationId);
			throw error;
		}
	}
	/**
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
	...
	*/
	async handleNativeResponse(response, request, reqTimestamp) {
		...
		const baseAccount = buildAccountToCache(this.browserStorage, authority, homeAccountIdentifier, base64Decode, this.correlationId, idTokenClaims, response.client_info, void 0, idTokenClaims.tid, void 0, response.account.id);
	...
	*/
	cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, responseAccessToken, tenantId, reqTimestamp) {
		...
		const tokenExpirationSeconds = reqTimestamp + (request.tokenType === AuthenticationScheme$1.POP ? SHR_NONCE_VALIDITY : (typeof response.expires_in === "string" ? parseInt(response.expires_in, 10) : response.expires_in) || 0);
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/config/Configuration.mjs
...
var PlatformAuthDOMHandler = class PlatformAuthDOMHandler {
	...
	*/
	...
	getDOMExtraParams(extraParameters) {
		return { ...Object.entries(extraParameters).reduce((record, [key, value]) => {
...
var SilentIframeClient = class extends StandardInteractionClient {
	...
	*/
	async silentTokenHelper(authClient, request) {
		...
		if (request.httpMethod === HttpMethod.POST) await invokeAsync(initiateCodeFlowWithPost, SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(this.config, authClient.authority, silentRequest, this.logger, this.performanceClient);
		else {
...
var StandardController = class StandardController {
	...
	*/
	acquireTokenPopup(request) {
		...
		if (this.canUsePlatformBroker(request)) result = this.acquireTokenNative({
			...request,
			correlationId
	...
	*/
	async ssoSilent(request) {
		...
		let result;
		if (this.canUsePlatformBroker(validRequest)) result = this.acquireTokenNative(validRequest, ApiId.ssoSilent).catch((e) => {
			if (e instanceof NativeAuthError && isFatalNativeAuthError(e)) {
				this.platformAuthProvider = void 0;
...
function checkIfRefreshTokenErrorCanBeResolvedSilently(refreshTokenError, cacheLookupPolicy) {
	const noInteractionRequired = !(refreshTokenError instanceof InteractionRequiredAuthError && refreshTokenError.subError !== badToken);
	...
	*/
	fromBridgeError(error) {
		if (isBridgeError(error)) switch (error.status) {
			...
			case BridgeStatusCode.NestedAppAuthUnavailable: return new ClientAuthError(error.code || nestedAppAuthBridgeDisabled, error.description);
...
async function loadAccount(request, clientInfo, correlationId, storage, logger, cryptoObj, authority, idTokenClaims) {
	...
	const cachedAccount = buildAccountToCache(storage, authority, homeAccountId, base64Decode, correlationId, idTokenClaims, clientInfo, authority.getPreferredCache(), claimsTenantId, void 0, void 0, logger);
...
function getPerfMeasurementModule() {
	...
	try {
		...
		if (Number(perfEnabled) === 1) return import("./BrowserPerformanceMeasurement-MGF07V7p.js").then((n) => n.n);
	} catch (e) {}
}
/**
