...
const PhasePanel = ({ phaseKey, phaseData, pl, isForecast, planData, actualData, updSec, updField, admin, t }: PhasePanelProps) => {
  ...
  const fcInfo = useMemo(() => {
    ...
    // Use actual revenue/cost from entries if available; otherwise estimate from MM ratio
  ...
  const combinedMetrics = useMemo(() => {
    ...
    // Revenue: use actual entries if available, else estimate from MM × unit rate
    const actRev = fcInfo.actRevActual;
    // Direct cost: use actual entries if available, else estimate from planning cost ratio
...
const ActualDataScreen = ({ project, setProjects, t }: ActualDataScreenProps) => {
  ...
  // Find "Direct Cost" total column: exact match only (not sub-items with " - ")
        ...
        <div className="mb-6 bg-gray-900 rounded-xl border border-teal-700/50 overflow-hidden">
          <div className="px-4 py-3 bg-teal-950/40 border-b border-teal-700/40 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">{staged.allRows.length} dòng · {staged.codes.length} project code tìm thấy</p>
            </div>
