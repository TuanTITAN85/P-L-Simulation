...
import { api } from "../api";
import type { DclUser } from "../types";
...
function Modal({
  ...
  const [title, setTitle]   = useState<"DCL" | "Vice DCL">(editUser?.dclTitle ?? "DCL");
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        ...
        await api.updateDclUser(editUser.id, { name: name.trim(), email: email.trim(), status, title });
      } else {
        ...
        await api.createDclUser({ name: finalName, email: addEmail.trim(), title });
      }
      onSaved(); onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <h2 className="font-semibold text-white">{isEdit ? t.editDcl : t.addDcl}</h2>
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          ...
          {/* Title — always shown */}
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t.dclTitle}</label>
            <div className="flex gap-2">
              {(["DCL", "Vice DCL"] as const).map(opt => (
                <button key={opt} type="button" onClick={() => setTitle(opt)}
                  className={`flex-1 py-2 text-sm rounded-lg border transition ${
                    title === opt ? "border-indigo-500 bg-indigo-900/40 text-indigo-200" : "border-gray-700 bg-gray-800 text-gray-400 hover:border-gray-600"
                  }`}>
                  {opt === "DCL" ? t.dcl : t.viceDcl}
                </button>
              ))}
            </div>
          </div>
...
export const DclManagementScreen = ({ t }: Props) => {
  const [users, setUsers]         = useState<DclUser[]>([]);
  const [loading, setLoading]     = useState(true);
  const [filter, setFilter]       = useState<Filter>("all");
  const [search, setSearch]       = useState("");
  const [modalUser, setModalUser] = useState<DclUser | null | undefined>(undefined);
  const { toast, show: showToast } = useToast();

  const load = async () => {
    try { setUsers(await api.getDclUsers()); }
  ...
  const handleDelete = async (u: DclUser) => {
    if (!window.confirm(`${t.confirmDelete}: "${u.name}"?`)) return;
    try {
      await api.deleteDclUser(u.id);
      showToast(t.savedOK);
      void load();
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-white">🔑 {t.dclMgmt}</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 text-gray-400 text-xs uppercase">
                ...
                <th className="text-left px-4 py-3">{t.dclTitle}</th>
                ...
                <tr key={u.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      u.dclTitle === "DCL" ? "bg-orange-900 text-orange-300" : "bg-amber-900 text-amber-300"
                    }`}>{u.dclTitle}</span>
                  </td>
