...
function Modal({
  ...
  const [lookedUpName, setLookedUpName] = useState<string | null>(null); // null = not found yet
  ...
  // Debounced email lookup (add mode only)
  useEffect(() => {
    if (isEdit) return;
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        ...
        await api.updatePmUser(editUser.id, { name: name.trim(), email: email.trim(), status });
      } else {
        ...
        await api.createPmUser({ name: finalName, email: addEmail.trim() });
      }
      onSaved(); onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <h2 className="font-semibold text-white">{isEdit ? t.editPm : t.addPm}</h2>
...
function CsvImport({ onDone, t }: { onDone: () => void; t: TranslationType }) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await api.importPmUsers(rows);
...
// ── Main screen ────────────────────────────────────────────────────────────────
export const PmManagementScreen = ({ t }: Props) => {
  const [users, setUsers]         = useState<PmUser[]>([]);
  const [loading, setLoading]     = useState(true);
  const [filter, setFilter]       = useState<Filter>("all");
  const [search, setSearch]       = useState("");
  const [modalUser, setModalUser] = useState<PmUser | null | undefined>(undefined);
  const [showImport, setShowImport] = useState(false);
  const { toast, show: showToast } = useToast();

  const load = async () => {
    try { setUsers(await api.getPmUsers()); }
  ...
  const handleDelete = async (u: PmUser) => {
    if (!window.confirm(`${t.confirmDelete}: "${u.name}"?`)) return;
    try {
      await api.deletePmUser(u.id);
      showToast(t.savedOK);
      void load();
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-white">👤 {t.pmMgmt}</h1>
        <div className="flex gap-2">
              ...
              {filtered.map(u => (
                <tr key={u.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs px-1.5 py-0.5 rounded font-medium bg-indigo-900 text-indigo-300">PM</span>
                  </td>
                  <td className="px-4 py-3">
